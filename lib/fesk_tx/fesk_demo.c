...
static void analyze_transmission(const uint8_t* text, size_t len) {
    ...
    // Keep track of a copy paste friendly string where we simply comma separate the tones we emit.
    char tone_string[5000];
    tone_string[0] = '\0';  // Clear the tone string

    while ((tone = fesk_get_next_tone(&encoder)) != 255 && pos < 1000) {
        // Append the current tone to the tone string
        if (pos > 0) {
            strcat(tone_string, ",");
        }
        // The tone as a string.
        char tone_str[10];
        snprintf(tone_str, sizeof(tone_str), "%d", tone);
        strcat(tone_string, tone < 3 ? tone_str : "???");
    ...
    printf("Tone sequence (comma-separated): %s\n", tone_string);
    printf("=================\n");
}
